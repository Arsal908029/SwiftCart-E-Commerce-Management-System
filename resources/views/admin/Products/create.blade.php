...
.field input,.field textarea,.field select{background:var(--deep);border:1px solid var(--border);border-radius:10px;color:var(--text);padding:.75rem 1rem;font-size:.9rem;font-family:inherit;outline:none;width:100%;transition:border-color .2s;}
...
.btn-submit:hover{background:#00f0ff;transform:translateY(-2px);}
...
<div class="wrap">
    ...
    <h1 class="page-title">Add <span>Product</span></h1>
    @if($errors->any())<div class="alert-danger">@foreach($errors->all() as $e)<div>{{ $e }}</div>@endforeach</div>@endif
    <form action="{{ url('/admin/products/store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-card">
            ...
            <div class="field">
                ...
                <textarea name="description" rows="4" required placeholder="Describe the product...">{{ old('description') }}</textarea>
            </div>
            <div class="row2">
                <div class="field">
            ...
            <div class="field">
                ...
                <input type="url" name="image_url" placeholder="https://example.com/image.jpg" oninput="previewUrl(this)">
            </div>
            <img id="imgPreview" class="img-preview" alt="Preview">
        </div>
        <button type="submit" class="btn-submit"><i class="fas fa-plus me-2"></i> Add Product</button>
    </form>
</div>
<script>
